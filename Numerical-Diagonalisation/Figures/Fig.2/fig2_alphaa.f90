...
! for the \( (\alpha, a) \)-family of jump-distributions. The program saves the
...
program fig2_alphaa
    ...
    real*8 :: alpha, a, L, lambda, dl, w_alphaa, sum, x, R_fun_alphaa
    double precision :: phi(N)     ! Array to store the eigenvector

    ! Parameters
    parameter(N = 2000, a = 1.75d0, alpha = 6.0d0, L = 10.d0)  ! Set grid size, parameter values, and system size

    ! Open output file to store eigenvector data
    open(1, file = "eigenvector_alphaa.dat", status = "unknown")
